...
async function fetchBooks(): Promise<EnrichedBook[]> {
    try {
        await authenticate();
        const records = await pb.collection("shisho_books").getFullList<ShishoBookRecord>();
        ...
        for (const pbBook of records) {
            const cleanIsbn = (pbBook.isbn || "").replace(/[^a-zA-Z0-9]/g, "").trim() || pbBook.id;
            ...
            if (imageUrl && cleanIsbn) {
                ...
                const localFilePath = path.join(COVERS_DIR, localFileName);
